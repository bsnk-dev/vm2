...
return ((vm, host) => {
	...
	for (let i = 0; i < vm.options.sourceExtensions.length; i++) {
		...
		EXTENSIONS['.' + ext] = (module, filename, dirname) => {
			if (vm.options.require.context !== 'sandbox') {
				try {
					if (vm.options.require.allowCaching === false) delete host.require.cache[filename];

					// We already have everything resolved, so we can just load in the module or JSON
					// and wrap it so it uses the mocked modules
					patchedRequire.cache = Object.create(null);
					patchedRequire.BUILTIN = Object.create(null);

					function patchedRequire(name) {
						const originalName = name;

						if (vm.options.require.resolve) {
							name = vm.options.require.resolve(name);
						}
						...
						if (exists) {
							ext = pa.extname(name);
							isDirectory = fs.statSync(name).isDirectory();
						}

						if (isDirectory) {
							const hasPackageJson = fs.existsSync(pa.join(name, 'package.json'));

							if (hasPackageJson) {
								const packageJSON = parseJSON(fs.readFileSync(pa.join(name, 'package.json'), 'utf8'));
								const main = packageJSON.main || 'index.js';
								name = pa.join(name, main);
								ext = pa.extname(name);

								if (pa.extname(name) === '') {
									if (fs.existsSync(pa.join(name, 'index.js'))) {
										name = pa.join(name, 'index.js');
									} else {
										name += '.js';
									}
						...
						// if the name is .node or .json we need to load it as a native module
						if (ext === '.node') {
						...
						for (const mocked in vm.options.require.mock) {
							if (originalName === mocked) return vm.options.require.mock[mocked];
						}

						// if it doesn't exist at this point we need to load it as a native module
						if (!fs.existsSync(name)) {
							if (originalName === 'internal/errors') {
								let script;
								try {
									script = new Script(`(function (exports, require, module, process, internalBinding) {
                      'use strict';
                      const primordials = global;
                      ${BUILTIN_MODULES[originalName]}
                      \n
                    });`, {
										filename: `${originalName}.vm.js`
									});
								...
								// setup module scope
								const module = BUILTINS[originalName] = {
									exports: {},
								...
								try {
									...
									throw new VMError(`Error loading '${originalName}'`);
								}
								return module.exports;
							} else if (originalName === 'events') {
								return Contextify.readonly(host.require('jsEvents'));
							}

							return host.require(originalName);
						}
                ...
                (() => {
                  ...
                  const _require = (name) => {
                    if (name.startsWith('/') || name.startsWith('.')) {
                      return oldRequire(\`${name.split(/[\\\\/]+/g).reverse().slice(1).reverse().join('/')}\`+'/'+name);
                    } else {
              ...
              `;
							...
							// eslint-disable-next-line no-new-func
							const wrapper = Function('require, exports, module', patchedCode);
							wrapper(patchedRequire, module.exports, module);
						}
						return patchedRequire.cache[name].exports;
					}

					module.exports = Contextify.readonly(patchedRequire(filename));
				} catch (e) {
					throw Contextify.value(e);
				}
			} else {
	...
	const _requireBuiltin = (moduleName) => {
		...
		if (vm.options.require.allowModifyingBuiltin) {
			return host.require(moduleName);
		} else {
			return Contextify.readonly(host.require(moduleName));
		}
	};
	...
	const _prepareRequire = (currentDirname, parentAllowsTransitive = false) => {
		const _require = moduleName => {
			...
			if ((requireObj.allowCaching === undefined) || (requireObj.allowCaching === true) && CACHE[filename]) return CACHE[filename].exports;
			...
			if (EXTENSIONS[extname]) {
				...
				// This is where it gets the actual module from.
