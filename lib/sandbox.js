...
const patchedRequire = host.require('./patchedRequire');
const Union = host.require('unionfs').Union;
		...
		EXTENSIONS['.' + ext] = (module, filename, dirname) => {
			if (vm.options.require.context !== 'sandbox') {
				try {
					...
					let newRequire;
					if (vm.options.require.mock) {
						const fsUnion = new Union().use(fs);
						if (vm.options.require.mock.fs) fsUnion.use(vm.options.require.mock.fs);

						newRequire = patchedRequire.createPatchedRequire(fsUnion, vm.options.require.resolve, vm.options.require.mock || {});
					} else {
						newRequire = host.require;
					}

					module.exports = Contextify.readonly(newRequire(filename));
				} catch (e) {
					throw Contextify.value(e);
				}
			} else {
